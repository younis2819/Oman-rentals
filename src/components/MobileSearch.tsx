...
export default function MobileSearch() {
  ...
  // Search State
  const [category, setCategory] = useState('car') // 'car' or 'heavy'
  ...
  const handleSearch = () => {
    ...
    // CHANGED: Push to Homepage ('/') instead of '/search'
    router.push(`/?${params.toString()}`) 
  }

  return (
    <>
      {/* 1. THE TRIGGER (Sits on Homepage) */}
      <div 
        onClick={() => setIsOpen(true)}
        className="mx-4 mt-6 bg-white rounded-full shadow-lg border border-gray-100 p-4 flex items-center gap-4 active:scale-95 transition-transform cursor-pointer"
      >
      ...
      {/* 2. THE MODAL (Opens when clicked) */}
      {isOpen && (
        <div className="fixed inset-0 z-[60] bg-gray-50 flex flex-col animate-in slide-in-from-bottom-10 duration-200">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8">
            <div>
              <label className="text-xs font-bold text-gray-400 uppercase mb-3 block">I am looking for</label>
            ...
            <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm">
               <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="font-bold text-gray-900">When do you need it?</span>
               </div>
               <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input 
                      type="date" 
                      className="w-full bg-gray-50 p-3 rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-black"
          ...
          <div className="p-4 bg-white border-t border-gray-100 pb-8">
            <button 
              ...
              className="w-full bg-black text-white font-bold py-4 rounded-xl text-lg shadow-xl flex justify-center items-center gap-2"
            >
              <Search className="w-5 h-5" />
              Search {category === 'car' ? 'Cars' : 'Machinery'}
            </button>
          </div>
        </div>
      )}
